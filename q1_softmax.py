...
def softmax(x):
  ...
  max_x = tf.expand_dims(tf.reduce_max(x, axis=1), 1)
  #print max_x
  stable_x = tf.subtract(x, max_x)
  exp_x = tf.exp(stable_x)
  sum_exp_x = tf.expand_dims(tf.reduce_sum(exp_x, axis=1), 1)

  out = tf.divide(exp_x, sum_exp_x)
