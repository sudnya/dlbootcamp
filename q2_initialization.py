...
def xavier_weight_init():
  ...
  def _xavier_initializer(shape, **kwargs):
    ...
    xavier_range = np.sqrt(6.0 / (np.sum(shape) ))
    out          = tf.random_uniform(shape, -xavier_range, xavier_range)
    #print "output shape xavier ", out.shape
    ### END YOUR CODE
    return out
...
def test_initialization():
  ...
  test_dims = (100, 64)
  xavier_initializer = xavier_weight_init()
  out = xavier_initializer(test_dims)
  assert out.get_shape() == test_dims

  s   = tf.Session()
  ref = np.sqrt(6.0/np.sum(test_dims))
  y   = np.max(s.run(out))
