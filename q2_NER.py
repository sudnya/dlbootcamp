...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      #raise NotImplementedError
      L = tf.get_variable("L", shape=[len(self.wv), self.config.embed_size], initializer=xavier_weight_init())
      #this_window = tf.nn.embedding_lookup(self.wv, self.input_placeholder)
      this_window = tf.nn.embedding_lookup(params=L, ids=self.input_placeholder)
      #print "placeholder returned of shape ", this_window.shape
      window =  tf.reshape(this_window, (-1, self.config.window_size*self.config.embed_size)) 
      ### END YOUR CODE
      return window
  ...
  def add_model(self, window):
    ...
    ### YOUR CODE HERE
    #raise NotImplementedError
    dim_W1 = (self.config.window_size*self.config.embed_size, self.config.hidden_size)
    dim_W2 = (self.config.hidden_size, self.config.label_size)

    dim_b1 = (self.config.hidden_size,)
    dim_b2 = (self.config.label_size,)
  
    xavier_initializer = xavier_weight_init()

    with tf.variable_scope("Layer"):
        W  = xavier_initializer(dim_W1)
        tf.add_to_collection("regS", tf.reduce_sum(W**2))
        b1  = tf.Variable(tf.zeros(dim_b1), dtype=tf.float32, name="b1")
        a0 = tf.matmul(window,W) + b1
        tf.nn.dropout(a0, self.dropout_placeholder)

    with tf.variable_scope("Softmax"):
        U = xavier_initializer(dim_W2)
        tf.add_to_collection("regS", tf.reduce_sum(U**2))
        b2  = tf.Variable(tf.zeros(dim_b2), dtype=tf.float32, name="b2")
        output = tf.matmul(a0, U) + b2
        tf.nn.dropout(output, self.dropout_placeholder)
  ...
  def add_loss_op(self, y):
    ...
    ### YOUR CODE HERE
    #raise NotImplementedError
    #stable_logits = tf.reduce_mean(y, 1)
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=y)) + 0.5*self.config.l2*tf.reduce_sum(tf.get_collection("regS"))
